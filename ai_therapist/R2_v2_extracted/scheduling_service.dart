...
enum AppointmentStatus { scheduled, completed, cancelledByPatient, cancelledByClinician, noShow, rescheduled }
  ...
  List<DateTime> generateFrom(DateTime from) {
    ...
    while (true) {
      ...
      cursor = switch (pattern) {
        ...
        RecurrencePattern.monthly   => DateTime(cursor.year, cursor.month + 1, cursor.day),
...
class Appointment {
  ...
  bool get isUpcoming =>
      status == AppointmentStatus.scheduled && startTime.isAfter(DateTime.now());
  ...
  Map<String, dynamic> toJson() => {
    ...
    'clinicianId': clinicianId, 'type': type.name,
    ...
    'location': location, 'notes': notes, 'isOnline': isOnline,
  };

  factory Appointment.fromJson(Map<String, dynamic> j) => Appointment(
    id: j['id'], patientId: j['patientId'], patientName: j['patientName'],
    ...
    attendance: j['attendance'] != null
        ? AttendanceRecord.fromJson(j['attendance']) : null,
    reminderChannels: (j['reminderChannels'] as List)
        .map((c) => ReminderChannel.values.byName(c)).toList(),
  ...
  Appointment copyWith({
    ...
    id: id, patientId: patientId, patientName: patientName,
    clinicianId: clinicianId, type: type, startTime: startTime,
    duration: duration,
    status: status ?? this.status,
    recurrence: recurrence, seriesId: seriesId,
...
class AvailabilityBlock {
  ...
  Map<String, dynamic> toJson() => {
    'clinicianId': clinicianId, 'weekday': weekday,
    'startHour': startHour, 'startMinute': startMinute,
    'endHour': endHour, 'endMinute': endMinute,
    'isRecurring': isRecurring,
  };

  factory AvailabilityBlock.fromJson(Map<String, dynamic> j) => AvailabilityBlock(
    clinicianId: j['clinicianId'], weekday: j['weekday'],
    startHour: j['startHour'], startMinute: j['startMinute'],
    endHour: j['endHour'], endMinute: j['endMinute'],
...
class SchedulingService {
  ...
  static Future<List<Appointment>> getAllAppointments(String clinicianId) async {
  ...
  static Future<List<Appointment>> getAppointmentsForDate(
    String clinicianId, DateTime date,
  ) async {
  ...
  static Future<List<Appointment>> getUpcomingAppointments(
    String clinicianId, {int limit = 10}
  ) async {
    final all = await getAllAppointments(clinicianId);
  ...
  static Future<List<AvailabilityBlock>> getAvailability(
    ...
    return _store.readList<AvailabilityBlock>(
      _availabilityKey,
      (j) => AvailabilityBlock.fromJson(j as Map<String, dynamic>),
    ).then((list) => list.where((b) => b.clinicianId == clinicianId).toList());
  }

...
class _UniversalAssessmentScreenState extends State<UniversalAssessmentScreen> {
  ...
  Future<void> _submitAssessment() async {
    ...
    try {
       final normalizedAnswers = <int, int>{};
       _answers.forEach((key, value) {
         if (value is! num) return;
         final match = RegExp(r'(\d+)$').firstMatch(key);
         final questionIndex = int.tryParse(match?.group(1) ?? key);
         if (questionIndex != null) {
           normalizedAnswers[questionIndex] = value.toInt();
         }
       });

       final result = AssessmentService.calculateScore(
         widget.templateId,
         normalizedAnswers,
       );
       ...
       final response = await ApiClient.instance.post(
          ...
          body: {
            ...
            'assessment_id': widget.templateId,
